...
export class SinusoidalRenderer {
  constructor(canvas, opts = {}) {
    ...
    this._timeLastGridTime = 0;  // AJOUT : Initialisation de la variable (BUG ligne 53)
  ...
  attachAnalyser(analyser) { 
    if (!analyser) {
      Logger.warn('[SinusoidalRenderer] Analyser null fourni à attachAnalyser()');
    }
    this.analyser = analyser; 
    Logger.info('[SinusoidalRenderer] Analyser attaché', { hasAnalyser: !!analyser });
  }

  start() {
    if (this.running) {
      Logger.warn('[SinusoidalRenderer] Déjà en cours, ignoré');
      return;
    }
    ...
    requestAnimationFrame(this._loop);
    Logger.info('[SinusoidalRenderer] Boucle de rendu démarrée');
  }

  stop() { 
    if (this.running) {
      this.running = false; 
      Logger.info('[SinusoidalRenderer] Boucle de rendu arrêtée');
    }
  }

  clear() { 
    Logger.info('[SinusoidalRenderer] Effacement du canvas');
    this._drawBackdrop(); 
  }
