...
let mp3Worker = {
  lamejs: null,
  async loadLame() {
    if (this.lamejs) return;
    try {
      self.importScripts('https://cdn.jsdelivr.net/npm/lamejs@1.2.0/lame.min.js');
      this.lamejs = self.lamejs;
      if (!this.lamejs) throw new Error('lamejs undefined');
    } catch (err) {
...
self.onmessage = async function(e) {
  try {
    ...
    await mp3Worker.loadLame();
    
    const pcm16Data = new Int16Array(wavData);
    const encoder = new mp3Worker.lamejs.Mp3Encoder(1, 48000, 128);
    const mp3Data = [];
